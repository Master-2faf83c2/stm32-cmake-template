...
static uint8_t tx_buff[DEBUG_TX_SIZE];
static uint8_t rx_buff[DEBUG_RX_SIZE];

DebugRingBuf_t debug_cb;

static void debugRingBufInit(void){
    RingBufferInit(&debug_cb.rx, rx_buff, DEBUG_RX_SIZE);
    RingBufferInit(&debug_cb.tx, tx_buff, DEBUG_TX_SIZE);
...
static void debugTxCallback(void){
    if (debug_cb.tx.DataOut != debug_cb.tx.DataIn){
        ...
        HAL_UART_Transmit_DMA(&huart2,debug_cb.tx.DataOut->start, debug_cb.tx.DataOut->end - debug_cb.tx.DataOut->start + 1);
        debug_cb.tx.DataOut ++;
...
static void debugRxCallback(uint16_t pos){
    ...
    HAL_UARTEx_ReceiveToIdle_DMA(&huart2, debug_cb.rx.DataIn->start, DEBUG_RX_SIZE - debug_cb.rx.Counter);
}
